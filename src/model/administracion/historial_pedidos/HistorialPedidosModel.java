...
public class HistorialPedidosModel {
	...
	public DefaultTableModel getModelPedidos() {
		...
		try {
			...
			resultSet = statement.executeQuery("select p.\"ID\", p.PRECIO, p.EN_CURSO, p.FECHA, m.NOMBRE from " + DatabaseConnectionService.getDatabase() + ".PEDIDO p, " + DatabaseConnectionService.getDatabase() + ".MESA m where p.ID_MESA = m.\"ID\" order by id desc");
